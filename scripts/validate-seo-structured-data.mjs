...
function validateWebsite(node, route) {
  if (!node) return;
  if (node['@id'] !== websiteId) failures.push(`${route}: WebSite @id must be ${websiteId}`);
  if (node.url !== rootUrl) failures.push(`${route}: WebSite.url must be ${rootUrl}`);
  requireNonEmptyString(node.name, route, 'WebSite.name');
  requireNonEmptyString(node.description, route, 'WebSite.description');

  if (node.inLanguage !== 'es-AR') failures.push(`${route}: WebSite.inLanguage must be es-AR`);
...
function validateSoftware(node, route, canonical, title, description) {
  if (!node) return;
...
for (const file of htmlFiles) {
  ...
  if (htmlLang !== 'es-AR') failures.push(`${route}: html lang must be es-AR`);
  ...
  if (route === '/404.html') {
    ...
    if (robots.length !== 1 || !/\bnoindex\b/i.test(robots[0])) {
      failures.push('/404.html: robots meta must include noindex');
    }
    if (extractCanonical(content).length !== 0) {
      failures.push('/404.html: canonical must be omitted');
    }
    continue;
  }
  ...
  pageIdentities.push({ route, title, description });
  ...
  if (route.startsWith('/projects/')) {
    if (softwareNodes.length !== 1) {
      ...
      validateSoftware(softwareNodes[0], route, canonical, title, description);
    }
...
for (const page of pageIdentities) {
  if (titleOwners.has(page.title)) {
    failures.push(`${page.route}: duplicates title used by ${titleOwners.get(page.title)}`);
  } else {
    titleOwners.set(page.title, page.route);
  }

  if (descriptionOwners.has(page.description)) {
    failures.push(`${page.route}: duplicates description used by ${descriptionOwners.get(page.description)}`);
  } else {
...
if (failures.length > 0) {
  console.error('SEO and structured data validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`SEO and structured data validation passed for ${htmlFiles.length} HTML pages.`);
